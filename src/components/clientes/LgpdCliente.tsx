...
export function LgpdCliente({ clienteId }: { clienteId: string }) {
  ...
  async function anonimizar() {
    ...
    if (
      !confirm(
        "Anonimizar os dados pessoais deste titular? É IRREVERSÍVEL. Os dados fiscais sob guarda legal são mantidos.",
      )
    )
      return;
    setOcupado(true);
  ...
  return (
    <section className="space-y-2 rounded-2xl border border-linha bg-white p-4 text-sm">
      ...
      <p className="text-xs text-cinza">
        Direito de acesso e portabilidade (relatório) e direito de exclusão (anonimização que respeita a guarda fiscal).
      </p>
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button
          disabled={ocupado}
          onClick={anonimizar}
          className="rounded-lg border border-negativo px-3 py-1.5 text-negativo disabled:opacity-60"
        >
          Anonimizar titular
        </button>
      </div>
