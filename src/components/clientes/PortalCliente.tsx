  ...
  return (
    <section className="rounded-lg border border-linha bg-white p-4">
      ...
      <p className="mt-0.5 text-xs text-cinza">
        O cliente recebe um convite por e-mail, define a senha e passa a acessar o portal — onde vê apenas os
        documentos, notas fiscais, guias e boletos dele.
      </p>
      ...
      {acessos.length > 0 && (
        <ul className="mt-2 space-y-1">
          {acessos.map((a) => (
            <li
              key={a.id}
              className="flex items-center justify-between gap-2 rounded-lg border border-linha px-3 py-2 text-sm"
            >
              <span>
                <span className="font-medium text-texto">{a.nome}</span> <span className="text-cinza">· {a.email}</span>
              </span>
              <span className="flex items-center gap-3 text-xs">
                <span className={a.ativo ? "text-verde" : "text-cinza"}>{a.ativo ? "Ativo" : "Revogado"}</span>
                {a.ativo && (
                  <button disabled={ocupado} onClick={() => revogar(a.id)} className="text-negativo underline">
                    revogar
                  </button>
                )}
              </span>
      ...
      {erro && (
        <p role="alert" className="mt-1 text-xs text-negativo">
          {erro}
        </p>
      )}
    </section>
