...
export function TarefasCliente({ clienteId, tarefas }: { clienteId: string; tarefas: TarefaView[] }) {
  ...
  async function nova() {
    ...
    setOcupado(true);
    const r = await criarTarefa({ titulo, descricao: null, responsavelId: null, clienteId, departamento: null, prioridade: "media", prazo: null });
    setOcupado(false);
  ...
  return (
    <section className="rounded-lg border border-linha bg-white p-4">
      <div className="flex items-center justify-between">
        ...
        <Link href={`/tarefas?cliente=${clienteId}`} className="text-xs text-verde underline">ver todas</Link>
      </div>
      {tarefas.length === 0 ? (
        ...
        <ul className="mt-2 space-y-1">
          {tarefas.map((t) => (
            <li key={t.id} className="flex items-center justify-between gap-3 rounded-lg border border-linha px-3 py-2 text-sm">
      ...
      <div className="mt-3 flex gap-2">
        <input value={titulo} onChange={(e) => setTitulo(e.target.value)} placeholder="Nova tarefa para este cliente…" className="flex-1 rounded-lg border border-linha px-2 py-1.5 text-sm" />
        <button disabled={ocupado || !titulo.trim()} onClick={nova} className="rounded-lg bg-verde px-3 py-1.5 text-sm text-white disabled:opacity-60">Criar</button>
      </div>
    </section>
  );
}
