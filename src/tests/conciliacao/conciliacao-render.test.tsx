...
vi.mock("@/app/(app)/financeiro/conciliacao/conciliar-actions", () => ({ candidatosDoMovimento: vi.fn(), conciliarComBaixa: vi.fn(), conciliarComTitulo: vi.fn(), criarLancamento: vi.fn(), ignorarMovimento: vi.fn(), reabrirMovimento: vi.fn(), conciliarAutomaticos: vi.fn() }));
...
describe("Conciliacao", () => {
  it("renderiza seletor de conta, upload e a lista", () => {
    const html = renderToStaticMarkup(<Conciliacao contas={[{ id: "c1", nome: "Nubank" }]} inicio="2026-07-01" fim="2026-07-31" contaInicial="c1" movimentosIni={movs} categorias={[]} clientes={[]} fornecedores={[]} />);
