...
export function FormConstituicao({
  ...
  const [socios, setSocios] = useState<Socio[]>([
    { nome: "", cpf: "", participacao: "", papelSocietario: "administrador" },
  ]);
    ...
    <form onSubmit={enviar} className="space-y-4">
      <div className="rounded-2xl border border-linha bg-creme p-3">
        <label className="text-xs text-cinza">
          Formulário de constituição (PDF)
          <input
            type="file"
        ...
        <p className="mt-1 text-xs text-cinza">
          Opcional. O PDF fica anexado ao acervo do cliente. Preencha os campos abaixo à mão.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 text-sm">
        ...
        <label className="block">
          Logradouro
          <input name="logradouro" className={input} />
        </label>
        <label className="block">
          Número
          <input name="numero" className={input} />
        </label>
        <label className="block">
          Bairro
        ...
        <label className="block">
          UF
          <input name="uf" maxLength={2} className={input} />
        </label>
        <label className="block">
          CEP
          <input name="cep" className={input} />
        </label>
        <label className="col-span-2 block">
      ...
      <div className="space-y-2 rounded-2xl border border-linha bg-white p-3">
        <div className="flex items-center justify-between">
          ...
          <button
            type="button"
            onClick={() => setSocios([...socios, { nome: "", cpf: "", participacao: "", papelSocietario: "quotista" }])}
        ...
        {socios.map((s, i) => (
          <div key={i} className="flex flex-wrap items-center gap-2">
            ...
            {socios.length > 1 && (
              <button
                type="button"
                onClick={() => setSocios(socios.filter((_, idx) => idx !== i))}
