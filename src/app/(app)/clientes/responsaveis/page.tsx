...
export default async function ResponsaveisPage({ searchParams }: { searchParams: Promise<{ depto?: string; resp?: string; q?: string }> }) {
  ...
  let query = supabase.from("clientes").select("id, razao_social, cpf_cnpj").is("excluido_em", null).order("razao_social").limit(LIMITE);
  ...
  const { data: respRows } = ids.length
    ? await supabase.from("cliente_responsavel").select("cliente_id, usuario_id").eq("departamento", depto).in("cliente_id", ids)
    : { data: [] };
  const respPorCliente = new Map<string, string>((respRows ?? []).map((r) => [r.cliente_id as string, r.usuario_id as string]));

  const colaboradores = await listarColaboradores();

  // Filtro por responsável atual (em memória): id específico, "nenhum" (sem responsável) ou "" (qualquer).
  const lista = clientes
    .map((c) => ({ id: c.id as string, razaoSocial: (c.razao_social as string) ?? "—", cpfCnpj: (c.cpf_cnpj as string) ?? "", responsavelId: respPorCliente.get(c.id as string) ?? null }))
    .filter((c) => (resp === "" ? true : resp === "nenhum" ? c.responsavelId === null : c.responsavelId === resp));

  return (
    <main className="mx-auto max-w-5xl space-y-5 p-4">
      <PageHeader titulo="Responsáveis por departamento" subtitulo="Redistribuição de carteira — filtre, marque e atribua em massa" />
