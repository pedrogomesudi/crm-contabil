...
export async function gerarCompetencia(
  ano: number,
  mes: number,
): Promise<{ candidatas: number; clientes: number } | null> {
  if (!(await gate())) return null;
  const supabase = await createServerSupabase();
  return gerarInstancias(supabase, ano, mes);
}

export async function gerarCompetenciaCliente(
  clienteId: string,
  ano: number,
  mes: number,
): Promise<{ candidatas: number; clientes: number } | null> {
...
export async function gerarRetroativo(
  anoIni: number,
  mesIni: number,
  clienteId?: string,
): Promise<{ meses: number; candidatas: number } | null> {
  if (!(await gate())) return null;
  const supabase = await createServerSupabase();
...
export async function listarInstancias(
  ...
  let q = supabase
    ...
    .select(
      "id, competencia, vencimento_legal, vencimento_interno, status, responsavel_id, entregue_em, comprovante_path, obrigacao(nome, codigo, periodicidade, comprovante_obrigatorio), clientes!inner(razao_social), responsavel:responsavel_id(nome), entregador:entregue_por(nome)",
    )
    .gte("competencia", ini)
    .lte("competencia", fim)
  ...
  return (data ?? []).map((r) => {
    const o = um(
      r.obrigacao as
        | { nome?: string; codigo?: string; periodicidade?: string; comprovante_obrigatorio?: boolean }
        | { nome?: string; codigo?: string; periodicidade?: string; comprovante_obrigatorio?: boolean }[]
        | null,
    );
...
export async function listarRiscos(opts?: { soMeus?: boolean }): Promise<PainelRiscos> {
  ...
  let q = supabase
    .from("obrigacao_instancia")
    .select(
      "id, competencia, vencimento_legal, vencimento_interno, responsavel_id, entregue_em, obrigacao(nome, periodicidade), clientes!inner(razao_social), responsavel:responsavel_id(nome)",
    )
    .eq("status", "pendente")
    .is("entregue_em", null)
  ...
  const itens: ItemRisco[] = (data ?? []).map((r) => {
    const o = um(
      r.obrigacao as { nome?: string; periodicidade?: string } | { nome?: string; periodicidade?: string }[] | null,
    );
    const cl = um(r.clientes as { razao_social?: string } | { razao_social?: string }[] | null);
    const resp = um(r.responsavel as { nome?: string } | { nome?: string }[] | null);
    return {
...
export async function contarRiscos(): Promise<number> {
  ...
  const { data } = await supabase
    .from("obrigacao_instancia")
    .select("vencimento_interno, clientes!inner(id)")
    .eq("status", "pendente")
    .is("entregue_em", null)
    .eq("clientes.status", "ativo");
  const hoje = new Date().toLocaleDateString("en-CA", { timeZone: "America/Sao_Paulo" });
