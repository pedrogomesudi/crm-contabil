...
async function coletar(perfilId: string): Promise<ItemEscalado[]> {
  ...
  const { data } = await admin
    ...
    .select("id, vencimento_interno, responsavel_id, obrigacao(nome), clientes!inner(razao_social)")
    .eq("status", "pendente")
    .is("entregue_em", null)
    .eq("clientes.status", "ativo")
    .lt("vencimento_interno", hoje);
  ...
  for (const r of data ?? []) {
    ...
    out.push({
      ...
      diasAtraso,
      nivel: nivel as 1 | 2,
      responsavelNome: nomeMap.get(respId) ?? null,
    });
  }
