...
export function EditorTemplate({ template }: { template: Tpl }) {
  ...
  function salvarItem() {
    ...
    void chamar(() =>
      salvarTemplateItem({
        ...
        prazoDias: form.prazoDias ?? null,
        aplicavelA: form.aplicavelA ?? ["*"],
        condicaoFlags: form.condicaoFlags ?? [],
        condicaoModo: (form.condicaoModo ?? "all") as "any" | "all",
        bloqueante: form.bloqueante ?? false,
        anexoObrigatorio: form.anexoObrigatorio ?? false,
        alertaRisco: form.alertaRisco ?? null,
        ordem: form.ordem ?? 0,
  ...
  function salvarBlocoForm() {
    ...
    void chamar(() =>
      bloco.id
        ? salvarBloco(bloco.id, bloco.nome, bloco.prazoBlocoDias, bloco.ordem)
        : criarBloco(template.id, bloco.nome, bloco.prazoBlocoDias),
    );
  }
      ...
      <section className="space-y-2 rounded-2xl border border-linha bg-white p-4">
        ...
        <div className="flex justify-end">
          <Botao
            variante="secundario"
            disabled={ocupado}
            onClick={() => chamar(() => salvarTemplate(template.id, nome, descricao || null, ativo))}
          >
      ...
      {template.blocos.map((b) => (
        <div key={b.id} className="space-y-1.5 rounded-2xl border border-linha bg-white p-3">
          <div className="flex items-center gap-2">
            ...
            {b.prazoBlocoDias != null && (
              <span className="font-mono text-[11px] text-cinza-claro">D+{b.prazoBlocoDias}</span>
            )}
            <button
              type="button"
              onClick={() => chamar(() => moverBloco(b.id, "cima"))}
            ...
            <button
              type="button"
              onClick={() => chamar(() => moverBloco(b.id, "baixo"))}
              className="text-cinza-claro hover:text-verde"
            >
              ↓
            </button>
            <button
              type="button"
              onClick={() => setBloco({ id: b.id, nome: b.nome, prazoBlocoDias: b.prazoBlocoDias, ordem: b.ordem })}
            ...
            <button
              type="button"
              onClick={() => {
                if (confirm(`Remover o bloco "${b.nome}" e seus itens?`)) void chamar(() => removerBloco(b.id));
              }}
            ...
            <button
              ...
              onClick={() =>
                setForm({
                  ...
                  aplicavelA: ["*"],
                  condicaoModo: "all",
                  ordem: (b.itens.at(-1)?.ordem ?? 0) + 1,
                })
              }
              className="ml-auto text-xs text-cinza underline"
            >
              + item
            </button>
          </div>
          <div className="overflow-x-auto rounded-lg border border-linha">
            <table className="min-w-full text-sm">
              <tbody>
                {b.itens.map((i) => (
                  <tr key={i.id} className="border-b border-linha/60">
                    ...
                    <td className="px-2 py-2 text-texto">
                      {i.titulo}
                      {i.bloqueante && (
                        <span className="ml-2 rounded bg-negativo/10 px-1.5 text-[10px] text-negativo">bloq.</span>
                      )}
                      {i.tipo === "acesso" && (
                        <span className="ml-2 rounded bg-verde/10 px-1.5 text-[10px] text-verde">cofre</span>
                      )}
                    </td>
                    <td className="px-2 py-2 font-mono text-[11px] text-cinza">
                      {i.prazoDias != null ? `D+${i.prazoDias}` : "—"}
                    </td>
                    <td className="px-2 py-2 text-right whitespace-nowrap">
                      <button
                        type="button"
                        onClick={() => chamar(() => moverItem(i.id, "cima"))}
                        className="mr-1 text-cinza-claro hover:text-verde"
                      >
                        ↑
                      </button>
                      <button
                        type="button"
                        onClick={() => chamar(() => moverItem(i.id, "baixo"))}
                        className="mr-3 text-cinza-claro hover:text-verde"
                      >
                        ↓
                      </button>
                      ...
                      <button
                        type="button"
                        onClick={() => chamar(() => removerTemplateItem(i.id))}
                        className="text-xs text-negativo underline"
                      >
                        remover
                      </button>
                ...
                {b.itens.length === 0 && (
                  <tr>
                    <td colSpan={4} className="px-2 py-2 text-cinza-claro">
                      Sem itens.
                    </td>
                  </tr>
                )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
          <div className="w-full max-w-sm space-y-2 rounded-2xl bg-white p-5">
            <h3 className="font-display text-sm font-semibold text-texto">
              {bloco.id ? "Editar bloco" : "Novo bloco"}
            </h3>
            <label className="block text-xs text-cinza">
              Nome
              <input
            ...
            <div className="flex justify-end gap-2 pt-1">
              <Botao variante="fantasma" onClick={() => setBloco(null)}>
                Cancelar
              </Botao>
              <Botao variante="primario" disabled={ocupado || !bloco.nome.trim()} onClick={salvarBlocoForm}>
                Salvar
              </Botao>
            </div>
          </div>
          ...
          <div className="w-full max-w-md space-y-2 rounded-2xl bg-white p-5">
            ...
            <fieldset className="text-xs text-cinza">
              ...
              <label className="mr-3 inline-flex items-center gap-1">
                <input
                  type="checkbox"
                  checked={(form.aplicavelA ?? []).includes("*")}
                  onChange={(e) => setForm({ ...form, aplicavelA: e.target.checked ? ["*"] : [] })}
                />{" "}
                todos
              </label>
              {PERFIS.map((pf) => (
                <label key={pf} className="mr-3 inline-flex items-center gap-1">
                  <input
                    type="checkbox"
                    disabled={(form.aplicavelA ?? []).includes("*")}
                    checked={(form.aplicavelA ?? []).includes(pf)}
                    onChange={(e) =>
                      setForm({
                        ...form,
                        aplicavelA: e.target.checked
                          ? [...(form.aplicavelA ?? []).filter((x) => x !== "*"), pf]
                          : (form.aplicavelA ?? []).filter((x) => x !== pf),
                      })
                    }
                  />{" "}
            ...
            <div className="flex gap-4 text-xs text-cinza">
              ...
              <label className="inline-flex items-center gap-1">
                <input
                  type="checkbox"
                  checked={form.anexoObrigatorio ?? false}
                  onChange={(e) => setForm({ ...form, anexoObrigatorio: e.target.checked })}
                />{" "}
                Anexo obrigatório
              </label>
            </div>
            <div className="flex justify-end gap-2 pt-1">
              ...
              <Botao variante="primario" disabled={ocupado || !(form.titulo ?? "").trim()} onClick={salvarItem}>
                Salvar
              </Botao>
            </div>
          </div>
