  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button type="button" onClick={semear} className="rounded-lg border border-linha px-3 py-1.5 text-sm">
          Semear matriz padrão
        </button>
      ...
      {form && (
        <div className="space-y-2 rounded-2xl border border-linha bg-white p-3">
          ...
          <div className="flex flex-wrap gap-2">
            <select
              multiple
              value={form.aplicavelA}
              onChange={(e) => setForm({ ...form, aplicavelA: Array.from(e.target.selectedOptions, (o) => o.value) })}
              className={inp}
            >
              {PERFIS.map((p) => (
            ...
            <input
              placeholder="Flags (csv: tem_folha)"
              value={csv(form.condicaoFlags)}
              onChange={(e) => setForm({ ...form, condicaoFlags: parse(e.target.value) })}
              className={inp}
            />
            <select
              value={form.condicaoModo}
              onChange={(e) => setForm({ ...form, condicaoModo: e.target.value })}
              className={inp}
            >
            ...
            <input
              placeholder="UFs (csv)"
              value={csv(form.ufs)}
              onChange={(e) => setForm({ ...form, ufs: parse(e.target.value) })}
              className={inp}
            />
            <input
              placeholder="CNAE prefixos (csv)"
              value={csv(form.cnaePrefixos)}
              onChange={(e) => setForm({ ...form, cnaePrefixos: parse(e.target.value) })}
              className={inp}
            />
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <label className="text-sm text-cinza">
              ...
              <input
                type="number"
                value={form.vencDia}
                onChange={(e) => setForm({ ...form, vencDia: Number(e.target.value) })}
            ...
            <label className="text-sm text-cinza">
              ...
              <input
                type="number"
                value={form.vencMesOffset}
                onChange={(e) => setForm({ ...form, vencMesOffset: Number(e.target.value) })}
                className={`${inp} ml-1 w-16`}
              />
            </label>
            <label className="text-sm text-cinza">
              ...
              <input
                type="number"
                value={form.vencMes ?? ""}
                onChange={(e) => setForm({ ...form, vencMes: e.target.value ? Number(e.target.value) : null })}
                className={`${inp} ml-1 w-16`}
              />
            </label>
            <label className="text-sm text-cinza">
              ...
              <input
                type="number"
                value={form.prazoInternoDiasUteis}
                onChange={(e) => setForm({ ...form, prazoInternoDiasUteis: Number(e.target.value) })}
                className={`${inp} ml-1 w-16`}
              />
            </label>
            <label className="flex items-center gap-1 text-sm text-cinza">
              <input
                type="checkbox"
                checked={form.antecipa}
                onChange={(e) => setForm({ ...form, antecipa: e.target.checked })}
              />
              antecipa
            </label>
            <label className="flex items-center gap-1 text-sm text-cinza">
              <input
                type="checkbox"
                checked={form.comprovanteObrigatorio}
                onChange={(e) => setForm({ ...form, comprovanteObrigatorio: e.target.checked })}
              />
