  ...
  return (
    <div className="space-y-4">
      <div className="overflow-x-auto rounded-2xl border border-linha bg-white">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {modelos.length === 0 ? (
              <tr>
                <td colSpan={4} className="px-3 py-4 text-cinza">
                  Nenhum modelo.
                </td>
              </tr>
            ) : (
              modelos.map((m) => (
                <tr key={m.id} className="border-b border-linha/60 hover:bg-creme">
                  <td className="px-3 py-2">
                    <Link href={`/configuracoes/legalizacao/${m.id}`} className="text-verde underline">
                      {m.nome}
                    </Link>
                  </td>
      ...
      <div className="flex flex-wrap items-end gap-2 rounded-2xl border border-linha bg-creme p-3 text-sm">
        ...
        {erro && (
          <span role="alert" className="w-full text-xs text-negativo">
            {erro}
          </span>
        )}
      </div>
