...
export async function reordenarEtapa(id: string, direcao: "cima" | "baixo"): Promise<{ ok?: boolean; erro?: string }> {
  ...
  const q = supabase.from("legalizacao_template_etapa").select("id, ordem").eq("template_id", atual.template_id as string);
  const { data: vizinha } = await (direcao === "cima"
  ...
  await supabase.from("legalizacao_template_etapa").update({ ordem: vizinha.ordem }).eq("id", atual.id as string);
  await supabase.from("legalizacao_template_etapa").update({ ordem: atual.ordem }).eq("id", vizinha.id as string);
