...
import { ContasVencimento } from "./ContasVencimento";
import { listarContasVencimento } from "./actions";
...
export default async function ContasVencimentoPage() {
  const perfil = await getPerfilAtual();
  if (!perfil || !podeGerenciarFinanceiro(perfil.papel)) redirect("/");
  const { inicio, fim } = mesCorrente();
  const linhasIni = await listarContasVencimento("RECEBER", inicio, fim);
  return (
    <Container largura="estreita" className="space-y-5 p-4">
      <Voltar href="/financeiro/relatorios" />
      <PageHeader
        titulo="Contas a receber e a pagar"
        subtitulo="Títulos em aberto por período de vencimento, com exportação em PDF, Excel e CSV."
      />
      <ContasVencimento inicio={inicio} fim={fim} linhasIni={linhasIni} />
    </Container>
  );
}
