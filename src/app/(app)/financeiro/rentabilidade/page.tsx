    ...
    <main className="mx-auto max-w-6xl space-y-5 p-4">
      ...
      <form method="GET" className="flex flex-wrap items-end gap-2 rounded-2xl border border-linha bg-white p-3 text-sm">
        <label className="text-xs text-cinza">
          De
          <input type="date" name="de" defaultValue={de} className="mt-0.5 block rounded-lg border border-linha px-2 py-1.5 text-sm" />
        </label>
        <label className="text-xs text-cinza">
          Até
          <input type="date" name="ate" defaultValue={ate} className="mt-0.5 block rounded-lg border border-linha px-2 py-1.5 text-sm" />
        </label>
      ...
      {rel.semCustoCadastrado && (
        <p className="rounded-lg bg-amber-50 px-3 py-2 text-sm text-amber-800">
          Há apontamentos de colaboradores <strong>sem custo/hora cadastrado</strong> no período — o custo deles
          entrou como zero. Cadastre em{" "}
          <Link href="/configuracoes/custos" className="underline">Configurações → Custo por colaborador</Link>.
        </p>
      )}
      {semApontamento > 0 && (
        <p className="rounded-lg bg-amber-50 px-3 py-2 text-sm text-amber-800">
              ...
              return (
                <tr key={l.clienteId} className="border-b border-linha/60 hover:bg-creme">
                  <td className="px-3 py-2">
                    <Link href={`/clientes/${l.clienteId}`} className="text-verde underline">{l.clienteNome}</Link>
      ...
      <p className="text-xs text-cinza">
        <strong>Recebido</strong> = baixas não estornadas no período. <strong>Contratado</strong> = honorário
        mensal × meses do período — em âmbar quando o recebido ficou abaixo dele (sinal de atraso). O custo usa
        o valor/hora <strong>vigente na data de cada apontamento</strong>. Ordenado por margem: os piores
        primeiro.
      </p>
    </main>
  );
