      ...
      <section className="space-y-2 rounded-2xl border border-linha bg-white p-4 text-sm">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <label className="text-xs text-cinza">
            ...
            <select
              value={status}
              onChange={(e) => mudarStatus(e.target.value as TarefaStatus)}
              className={`mt-0.5 block w-full ${cls}`}
            >
              {TAREFA_STATUS.map((s) => (
      ...
      <section className="space-y-2 rounded-2xl border border-linha bg-white p-4 text-sm">
        ...
        {tarefa.itens.map((it) => (
          <div key={it.id} className="flex items-center gap-2">
            <input
              type="checkbox"
              checked={it.feito}
              onChange={(e) => run(() => alternarItem(it.id, e.target.checked))}
            />
            <span className={`flex-1 ${it.feito ? "text-cinza line-through" : "text-texto"}`}>{it.descricao}</span>
            <button onClick={() => run(() => excluirItem(it.id))} className="text-xs text-negativo underline">
              remover
            </button>
          </div>
        ))}
        <div className="flex gap-2">
          ...
          <button
            ...
            onClick={addItem}
            className="rounded-lg border border-linha px-3 py-1.5 disabled:opacity-60"
          >
            + item
          </button>
        </div>
