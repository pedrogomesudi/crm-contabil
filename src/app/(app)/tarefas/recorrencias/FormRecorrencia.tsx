...
export function FormRecorrencia({
  ...
  const mudar = (campo: keyof RecorrenciaView, valor: unknown) => setEdit((e) => (e ? { ...e, [campo]: valor } : e));
            ...
            >
              ...
              {editavel && (
                <span className="flex gap-3 text-xs">
                  <button onClick={() => abrir(r)} className="text-verde underline">
                    editar
                  </button>
                  <button
                    disabled={ocupado}
        ...
        <div className="space-y-3 rounded-2xl border border-linha bg-white p-4 text-sm">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <label className="text-xs text-cinza">
              ...
              <select
                ...
                onChange={(e) => mudar("prioridade", e.target.value as TarefaPrioridade)}
          ...
          <label className="block text-xs text-cinza">
            ...
            <textarea
              ...
              onChange={(e) => mudar("descricao", e.target.value || null)}
              rows={2}
              className={`mt-0.5 block w-full ${cls}`}
            />
          </label>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <label className="text-xs text-cinza">
              Departamento
              <select
                value={edit.departamento ?? ""}
                onChange={(e) => mudar("departamento", (e.target.value || null) as Departamento | null)}
                className={`mt-0.5 block ${cls}`}
              >
                <option value="">—</option>
                {DEPARTAMENTOS.map((d) => (
            ...
            <label className="text-xs text-cinza">
              ...
              <select
                ...
                onChange={(e) => mudar("responsavelId", e.target.value || null)}
                className={`mt-0.5 block ${cls}`}
              >
          ...
          <div className="flex flex-wrap gap-2">
            <label className="text-xs text-cinza">
              Periodicidade
              <select
                value={edit.periodicidade}
                onChange={(e) => mudar("periodicidade", e.target.value as Periodicidade)}
                className={`mt-0.5 block ${cls}`}
              >
            ...
            {edit.periodicidade === "semanal" ? (
              <label className="text-xs text-cinza">
                Dia da semana
                <select
                  value={edit.diaSemana ?? 1}
                  onChange={(e) => mudar("diaSemana", Number(e.target.value))}
                  className={`mt-0.5 block ${cls}`}
                >
                  {DIAS.map((d, i) => (
              ...
              <label className="text-xs text-cinza">
                ...
                <input
                  ...
                  max={31}
                  value={edit.diaMes ?? 1}
                  onChange={(e) => mudar("diaMes", Number(e.target.value))}
            ...
            {edit.periodicidade === "anual" && (
              <label className="text-xs text-cinza">
                ...
                <input
                  ...
                  max={12}
                  value={edit.mes ?? 1}
                  onChange={(e) => mudar("mes", Number(e.target.value))}
                  className={`mt-0.5 block w-20 ${cls}`}
                />
              </label>
            )}
            ...
            <label className="text-xs text-cinza">
              ...
              <input
                ...
                min={0}
                max={60}
                value={edit.antecedenciaDias}
                onChange={(e) => mudar("antecedenciaDias", Number(e.target.value))}
                className={`mt-0.5 block w-24 ${cls}`}
              />
              <span className="mt-0.5 block text-[11px] text-cinza-claro">
                Quantos dias antes do prazo a tarefa nasce.
              </span>
            </label>
            ...
            <label className="text-xs text-cinza">
              ...
              <input
                type="date"
                value={edit.proximaData}
                onChange={(e) => mudar("proximaData", e.target.value)}
                className={`mt-0.5 block ${cls}`}
              />
            </label>
          ...
          <div className="flex items-center gap-3">
            ...
            <button type="button" onClick={() => setEdit(null)} className="text-xs text-cinza underline">
              cancelar
            </button>
          </div>
        </div>
