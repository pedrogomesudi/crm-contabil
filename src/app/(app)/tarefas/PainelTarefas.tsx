...
export function PainelTarefas({ tarefas, colaboradores, filtros, vista, hoje }: {
  ...
  async function nova() {
    ...
    setOcupado(true);
    const r = await criarTarefa({ titulo, descricao: null, responsavelId: null, clienteId: filtros.cliente ?? null, departamento: null, prioridade: "media", prazo: null });
    setOcupado(false);
    ...
    <div className="space-y-4">
      <form method="GET" className="flex flex-wrap items-end gap-2 rounded-2xl border border-linha bg-white p-3 text-sm">
        <label className="text-xs text-cinza">Responsável
          <select name="responsavel" defaultValue={filtros.responsavel ?? ""} className="mt-0.5 block rounded-lg border border-linha px-2 py-1.5 text-sm">
            <option value="">Todos</option>
            {colaboradores.map((c) => <option key={c.id} value={c.id}>{c.nome}</option>)}
          </select>
        </label>
        <label className="text-xs text-cinza">Departamento
          <select name="departamento" defaultValue={filtros.departamento ?? ""} className="mt-0.5 block rounded-lg border border-linha px-2 py-1.5 text-sm">
            <option value="">Todos</option>
            {DEPARTAMENTOS.map((d) => <option key={d.valor} value={d.valor}>{d.rotulo}</option>)}
          </select>
        </label>
        <label className="text-xs text-cinza">Status
          <select name="status" defaultValue={filtros.status ?? ""} className="mt-0.5 block rounded-lg border border-linha px-2 py-1.5 text-sm">
            <option value="">Todos</option>
            {TAREFA_STATUS.map((s) => <option key={s.valor} value={s.valor}>{s.rotulo}</option>)}
          </select>
        </label>
        <label className="text-xs text-cinza">Prioridade
          <select name="prioridade" defaultValue={filtros.prioridade ?? ""} className="mt-0.5 block rounded-lg border border-linha px-2 py-1.5 text-sm">
      ...
      <div className="flex gap-2">
        <input value={titulo} onChange={(e) => setTitulo(e.target.value)} placeholder="Nova tarefa…" className="flex-1 rounded-lg border border-linha px-3 py-2 text-sm" />
      ...
      {vista === "lista" ? (
        <div className="overflow-x-auto rounded-2xl border border-linha bg-white">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {tarefas.length === 0 ? (
                <tr><td colSpan={6} className="px-3 py-4 text-cinza">Nenhuma tarefa.</td></tr>
              ) : tarefas.map((t) => {
                ...
                return (
                  <tr key={t.id} className="border-b border-linha/60 hover:bg-creme">
                    <td className="px-3 py-2"><Link href={`/tarefas/${t.id}`} className="text-verde underline">{t.titulo}</Link></td>
                    ...
                    <td className="px-3 py-2"><span className={`rounded-full px-2 py-0.5 text-xs ${PRIO_COR[t.prioridade]}`}>{rotPrio(t.prioridade)}</span></td>
                ...
                {tarefas.filter((t) => t.status === col).map((t) => {
                  ...
                  return (
                    <div key={t.id} className="rounded-lg border border-linha bg-white p-2 text-sm">
                      <Link href={`/tarefas/${t.id}`} className="font-medium text-texto hover:underline">{t.titulo}</Link>
                      ...
                      <div className="mt-1 flex items-center justify-between">
                        <span className={`rounded-full px-2 py-0.5 text-xs ${PRIO_COR[t.prioridade]}`}>{rotPrio(t.prioridade)}</span>
                        <span className="flex gap-1">
                          {idx > 0 && <button disabled={ocupado} onClick={() => mover(t.id, KANBAN[idx - 1]!)} className="px-1 text-cinza" aria-label="Voltar status">←</button>}
                          {idx < KANBAN.length - 1 && <button disabled={ocupado} onClick={() => mover(t.id, KANBAN[idx + 1]!)} className="px-1 text-verde" aria-label="Avançar status">→</button>}
                        </span>
                      </div>
                    </div>
                  );
