...
export async function listarProcessos(): Promise<ResumoProcesso[]> {
  ...
  const { data: procs } = await supabase
    ...
    .select("id, perfil, cliente_id, clientes(razao_social)");
  if (!procs || procs.length === 0) return [];
  const { data: itens } = await supabase
    .from("onboarding_processo_item")
    .select("processo_id, status, prazo, bloqueante")
    .in(
      "processo_id",
      procs.map((x) => x.id as string),
    );
  ...
  const out = procs.map((pr) => {
    ...
    return {
      processoId: pr.id as string,
      clienteId: pr.cliente_id as string,
      razaoSocial: (cli?.razao_social as string) ?? "—",
      perfil: pr.perfil as string,
      total: prog.total,
      concluidos: prog.concluidos,
      pct: prog.pct,
      concluido: prog.concluido,
      proximoPrazo: prog.proximoPrazo,
    };
  });
