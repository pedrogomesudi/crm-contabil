...
export type BlocoView = {
  ...
  ordem: number;
  nome: string;
  prazoBlocoDias: number | null;
  itens: ItemTemplateView[];
};
...
async function carregarBlocos(
  ...
  tpl: { id: string; slug: string; nome: string; descricao: string | null; ativo: boolean },
): Promise<NonNullable<TemplateView>> {
  ...
  const { data: itens } = await supabase
    ...
    .select(
      "id, bloco_id, codigo, titulo, descricao, tipo, responsavel_papel, prazo_dias, aplicavel_a, condicao_flags, condicao_modo, bloqueante, anexo_obrigatorio, alerta_risco, ordem, depende_de, campo_destino",
    )
    .in(
      "bloco_id",
      (blocos ?? []).map((b) => b.id as string),
    )
    .order("ordem");
...
export async function obterTemplate(templateId: string): Promise<TemplateView> {
  ...
  const { data: tpl } = await supabase
    ...
    .select("id, slug, nome, descricao, ativo")
    .eq("id", templateId)
    .maybeSingle();
  if (!tpl) return null;
  return carregarBlocos(
    supabase,
    tpl as { id: string; slug: string; nome: string; descricao: string | null; ativo: boolean },
  );
}
...
export async function listarTemplates(): Promise<TemplateResumo[]> {
  ...
  const { data: itens } = blocoIds.length
    ? await supabase.from("onboarding_template_item").select("bloco_id").in("bloco_id", blocoIds)
    : { data: [] as { bloco_id: string }[] };
  ...
  return tpls.map((t) => ({
    ...
    blocos: nBlocos.get(t.id as string) ?? 0,
    itens: nItens.get(t.id as string) ?? 0,
    processos: nProcs.get(t.id as string) ?? 0,
  }));
}
...
export async function criarTemplate(nome: string, descricao: string | null): Promise<{ id?: string; erro?: string }> {
  ...
  const { data, error } = await supabase
    .from("onboarding_template")
    .insert({ slug, nome: nome.trim(), descricao })
    .select("id")
    .single();
...
export async function salvarTemplate(
  ...
  const { error } = await supabase
    .from("onboarding_template")
    .update({ nome: nome.trim(), descricao, ativo })
    .eq("id", id);
...
export async function semearTemplatePadrao(): Promise<{ ok?: boolean; erro?: string }> {
  ...
  const { data: existe } = await supabase
    .from("onboarding_template")
    .select("id")
    .eq("slug", TEMPLATE_PADRAO.slug)
    .maybeSingle();
  if (existe) return { ok: true };
  const { data: tpl, error: e1 } = await supabase
    .from("onboarding_template")
    .insert({ slug: TEMPLATE_PADRAO.slug, nome: TEMPLATE_PADRAO.nome, descricao: TEMPLATE_PADRAO.descricao })
    .select("id")
    .single();
  ...
  for (const b of TEMPLATE_PADRAO.blocos) {
    const { data: bloco, error: e2 } = await supabase
      ...
      .insert({
        ...
        slug: `bloco-${b.ordem}`,
        nome: b.nome,
        prazo_bloco_dias: b.prazoBlocoDias,
      })
      .select("id")
      .single();
    ...
    const linhas = b.itens.map((i) => ({
      ...
      tipo: i.tipo,
      responsavel_papel: i.responsavelPapel,
      prazo_dias: i.prazoDias,
      aplicavel_a: i.aplicavelA,
      condicao_flags: i.condicaoFlags,
      condicao_modo: i.condicaoModo,
      bloqueante: i.bloqueante,
      anexo_obrigatorio: i.anexoObrigatorio,
      alerta_risco: i.alertaRisco,
      ordem: i.ordem,
      depende_de: i.dependeDe,
      campo_destino: i.campoDestino,
    }));
...
export async function salvarTemplateItem(input: {
  id?: string;
  blocoId: string;
  codigo: string;
  titulo: string;
  ...
  const row = {
    ...
    alerta_risco: input.alertaRisco,
    ordem: input.ordem,
    depende_de: input.dependeDe,
    campo_destino: input.campoDestino,
  };
  const { error } = input.id
    ? await supabase.from("onboarding_template_item").update(row).eq("id", input.id)
    : await supabase.from("onboarding_template_item").insert(row);
  return error ? { erro: "Falha ao salvar." } : { ok: true };
}
...
export async function criarBloco(
  ...
  const { error } = await supabase.from("onboarding_bloco").insert({
    template_id: templateId,
    nome: nome.trim(),
    prazo_bloco_dias: prazoBlocoDias,
    ordem,
    slug: `bloco-${ordem}`,
  });
...
export async function salvarBloco(
  ...
  const { error } = await supabase
    .from("onboarding_bloco")
    .update({ nome: nome.trim(), prazo_bloco_dias: prazoBlocoDias, ordem })
    .eq("id", id);
...
export async function moverBloco(id: string, direcao: "cima" | "baixo"): Promise<{ ok?: boolean; erro?: string }> {
  ...
  const { data: irmaos } = await supabase
    .from("onboarding_bloco")
    .select("id, ordem")
    .eq("template_id", bloco.template_id as string);
  const alvo = alvoTroca(
    (irmaos ?? []).map((b) => ({ id: b.id as string, ordem: b.ordem as number })),
    id,
    direcao,
  );
...
export async function moverItem(id: string, direcao: "cima" | "baixo"): Promise<{ ok?: boolean; erro?: string }> {
  ...
  const { data: irmaos } = await supabase
    .from("onboarding_template_item")
    .select("id, ordem")
    .eq("bloco_id", item.bloco_id as string);
  const alvo = alvoTroca(
    (irmaos ?? []).map((i) => ({ id: i.id as string, ordem: i.ordem as number })),
    id,
    direcao,
  );
