...
type Linha = { id: string; cliente: string; titulo: string; status: string; pct: number; proximoPrazo: string | null; orgaosPendentes: LegOrgao[] };
...
export function PainelLegalizacao({ linhas, filtros }: { linhas: Linha[]; filtros: { status: string; orgao: string } }) {
  return (
    <div className="space-y-4">
      <form method="GET" className="flex flex-wrap items-end gap-2 rounded-2xl border border-linha bg-white p-3 text-sm">
        ...
        <label className="text-xs text-cinza">Órgão pendente
          <select name="orgao" defaultValue={filtros.orgao} className="mt-0.5 block rounded-lg border border-linha px-2 py-1.5 text-sm">
      ...
      <div className="overflow-x-auto rounded-2xl border border-linha bg-white">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {linhas.length === 0 ? (
              <tr><td colSpan={5} className="px-3 py-4 text-cinza">Nenhum processo para os filtros.</td></tr>
            ) : linhas.map((l) => (
              <tr key={l.id} className="border-b border-linha/60 hover:bg-creme">
                <td className="px-3 py-2"><Link href={`/legalizacao/${l.id}`} className="text-verde underline">{l.cliente}</Link></td>
