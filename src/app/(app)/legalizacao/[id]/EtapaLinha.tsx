...
type Etapa = {
  ...
  orgao: LegOrgao; orgaoOutro: string | null; prazo: string | null; status: LegEtapaStatus;
  protocolo: string | null; protocoloEm: string | null;
  anexoObrigatorio: boolean; anexoUrl: string | null;
  ...
  return (
    <div className={`rounded-2xl border border-linha bg-white p-3 ${concluida ? "opacity-80" : ""}`}>
      ...
      <div className="mt-2 grid gap-2 sm:grid-cols-2">
        <label className="text-xs text-cinza">Protocolo
          <div className="mt-0.5 flex gap-1">
            <input value={protocolo} onChange={(e) => setProtocolo(e.target.value)} onBlur={() => protocolo !== (etapa.protocolo ?? "") && salvar({ protocolo: protocolo || null })} className="w-full rounded-lg border border-linha px-2 py-1.5 text-sm" />
          </div>
        </label>
        <label className="text-xs text-cinza">Data do protocolo
          <input type="date" value={protocoloEm} onChange={(e) => { setProtocoloEm(e.target.value); salvar({ protocoloEm: e.target.value || null }); }} className="mt-0.5 block w-full rounded-lg border border-linha px-2 py-1.5 text-sm" />
        </label>
        <label className="text-xs text-cinza">Prazo
          <input type="date" value={prazo} onChange={(e) => { setPrazo(e.target.value); salvar({ prazo: e.target.value || null }); }} className="mt-0.5 block w-full rounded-lg border border-linha px-2 py-1.5 text-sm" />
          {sev && <span className={`ml-1 text-[11px] ${SEV[sev]}`}>{sev === "em_breve" ? "vence em breve" : sev === "vencido" ? "vencido" : "muito atrasado"}</span>}
        </label>
        {etapa.orgao === "outro" && (
          <label className="text-xs text-cinza">Órgão (rótulo)
            <input value={orgaoOutro} onChange={(e) => setOrgaoOutro(e.target.value)} onBlur={() => orgaoOutro !== (etapa.orgaoOutro ?? "") && salvar({ orgaoOutro: orgaoOutro || null })} className="mt-0.5 block w-full rounded-lg border border-linha px-2 py-1.5 text-sm" />
          </label>
        )}
      </div>

      <label className="mt-2 block text-xs text-cinza">Observação
        <textarea value={obs} onChange={(e) => setObs(e.target.value)} onBlur={() => obs !== (etapa.observacao ?? "") && salvar({ observacao: obs || null })} rows={2} className="mt-0.5 block w-full rounded-lg border border-linha px-2 py-1.5 text-sm" />
      </label>

      <div className="mt-2 flex flex-wrap items-center gap-3 text-xs">
        {etapa.anexoUrl ? (
          <a href={etapa.anexoUrl} target="_blank" rel="noopener noreferrer" className="text-verde underline">ver comprovante</a>
        ...
        <form action={enviar} className="flex items-center gap-1">
          ...
          <button disabled={ocupado} className="rounded-lg border border-linha px-2 py-1 disabled:opacity-60">Anexar</button>
        </form>
        {etapa.avisarCliente && (
          <label className="ml-auto flex items-center gap-1 text-cinza">
            <input type="checkbox" checked={!!etapa.clienteAvisadoEm} onChange={(e) => salvar({ clienteAvisado: e.target.checked })} />
