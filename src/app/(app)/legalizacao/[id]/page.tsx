...
export default async function ProcessoLegalizacaoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: etapas } = await supabase
    ...
    .select(
      "id, ordem, titulo, descricao, orgao, orgao_outro, prazo, status, protocolo, protocolo_em, anexo_obrigatorio, anexo_path, avisar_cliente, cliente_avisado_em, observacao",
    )
    .eq("processo_id", id)
    .order("ordem");
