...
export default async function LegalizacaoPage({ searchParams }: { searchParams: Promise<{ status?: string; orgao?: string }> }) {
  const perfil = await getPerfilAtual();
  if (!perfil || !podeGerenciarLegalizacao(perfil.papel)) redirect("/");
  const sp = await searchParams;
  const status = (sp.status && (sp.status === "todos" || STATUS.has(sp.status as LegProcStatus)) ? sp.status : "em_andamento");
  const orgao = (sp.orgao && ORGAOS.has(sp.orgao as LegOrgao) ? sp.orgao : "") as LegOrgao | "";

  const supabase = await createServerSupabase();
  let query = supabase.from("legalizacao_processo").select("id, cliente_id, tipo, titulo, status").order("criado_em", { ascending: false }).limit(300);
  ...
  for (const e of etapas ?? []) {
    ...
    a.push({ status: e.status as LegEtapaStatus, prazo: (e.prazo as string | null) ?? null, orgao: e.orgao as LegOrgao });
