...
export function periodoBounds(
  ...
  if (g === "mes") {
    ...
    inicioY = Math.floor(tot / 12);
    inicioM0 = ((tot % 12) + 12) % 12;
    meses = 1;
...
export function metricasFunil(ops: OpMetrica[], etapas: Etapa[], inicio: string, fim: string): MetricasFunil {
  ...
  const fechados = ops.filter(
    (o) =>
      (o.etapa === "ganho" || o.etapa === "perdido") &&
      o.fechadoEm != null &&
      o.fechadoEm >= inicio &&
      o.fechadoEm < fim,
  );
  ...
  for (const o of fechados) {
    ...
    if (o.etapa === "ganho") {
      r.ganhos += 1;
      r.valorGanho += o.valorEstimado ?? 0;
    } else r.perdidos += 1;
    rmap.set(nome, r);
  }
  const porResponsavel = [...rmap.values()].sort((a, b) => b.valorGanho - a.valorGanho);
  const mmap = new Map<string, number>();
  for (const o of perdidosArr) {
    const mo = o.motivoPerda ?? "Sem motivo";
    mmap.set(mo, (mmap.get(mo) ?? 0) + 1);
  }
