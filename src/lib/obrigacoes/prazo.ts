...
const somaDias = (s: string, n: number) =>
  new Date(Date.parse(`${s}T00:00:00Z`) + n * 86400000).toISOString().slice(0, 10);
...
export function calcularVencimento(regra: RegraPrazo, competencia: string): { legal: string; interno: string } {
  ...
  const interno =
    regra.prazoInternoDiasUteis > 0 ? subtraiDiasUteis(legal, regra.prazoInternoDiasUteis, feriados) : legal;
