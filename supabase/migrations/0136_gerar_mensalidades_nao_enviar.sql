-- "Não enviar" (os dois canais de cobrança desligados) tira o cliente do ciclo automático:
-- gerar_mensalidades passa a pulá-lo também na geração de honorários e do 13º. É a condição
-- "pelo menos um canal ligado" — independente de tem_honorarios_recorrentes, que continua valendo.
-- create or replace: substitui a versão vigente (0121) mantendo o resto idêntico.
...
begin
  ...
  for r in
    ...
    where ct.status = 'ATIVO' and ct.data_inicio <= v_fim
      ...
      and coalesce((select (f.cobranca_whatsapp or f.cobranca_email) from clientes_financeiro f where f.cliente_id = ct.cliente_id), true)
  loop
  ...
  for r in
    ...
    where c.excluido_em is null and c.status = 'ativo'
      ...
      and (coalesce(f.cobranca_whatsapp, true) or coalesce(f.cobranca_email, true))
